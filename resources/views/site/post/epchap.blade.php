  ...
  <div class="col">
    ...
    <div class="text-center mb-3 d-flex-sm justify-content-center align-items-center">
      <div class="d-block d-sm-inline-block">
        ...
        {!! Form::select(null, $data->serverArray, null, array('class' =>'custom-select m-2', 'style'=>'width:110px;', 'onchange'=>'javascript:document.getElementById("player").src = this.value;')) !!}
      </div>

      @if(isset($data->epPrev))
        <input type="hidden" id="prev" value="{!! CommonUrl::getUrl2($post->slug, $data->epPrev->slug) !!}">
        <a href="{!! CommonUrl::getUrl2($post->slug, $data->epPrev->slug) !!}" class="btn btn-primary m-2" rel="prev"><i class="fa fa-arrow-left" aria-hidden="true"></i><span class="ml-2 d-none d-md-inline">Tập trước</span></a>
      @else
        <input type="hidden" id="prev" value="">
        <a class="btn btn-secondary m-2 disabled"><i class="fa fa-arrow-left" aria-hidden="true"></i><span class="ml-2 d-none d-md-inline">Tập trước</span></a>
      @endif

      {!! Form::select(null, $post->epchapArray, CommonUrl::getUrl2($post->slug, $data->slug), array('class' =>'custom-select m-2', 'style'=>'width:110px;', 'onchange'=>'javascript:location.href = this.value;')) !!}

      @if(isset($data->epNext))
        <input type="hidden" id="next" value="{!! CommonUrl::getUrl2($post->slug, $data->epNext->slug) !!}">
        <a href="{!! CommonUrl::getUrl2($post->slug, $data->epNext->slug) !!}" class="btn btn-primary m-2" rel="next"><span class="mr-2 d-none d-md-inline">Tập sau</span><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
      @else
        <input type="hidden" id="next" value="">
        <a class="btn btn-secondary m-2 disabled"><span class="mr-2 d-none d-md-inline">Tập sau</span><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
      @endif
    </div>
